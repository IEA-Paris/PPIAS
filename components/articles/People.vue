  ...
  <v-row class="mt-12" :class="{ 'mx:6': $vuetify.breakpoint.mdAndUp }">
    ...
    >
      <v-avatar size="160" class="mb-3">
        <OptimizedImage v-if="item.image" alt="Avatar" :src="item.image" height="160" :ratio="1" />
        <v-icon
          v-else
    ...
    <v-col cols="12" md="8" class="mx-3">
      ...
      <div v-if="$vuetify.breakpoint.smAndDown" class="flex-row justify-center mb-6">
        <v-tooltip v-for="social in getSocials(item)" :key="social.link" bottom>
          <template #activator="{ on, attrs }">
            <v-btn text icon color="primary" v-bind="attrs" :href="social.link" target="_blank" v-on="on">
        ...
        <template v-if="podcast">
          <v-btn outlined nuxt :href="'https://www.intercontinental-academia.org/blog/' + podcast">
      ...
      <template v-if="podcast && $vuetify.breakpoint.mdAndUp">
        <v-btn outlined class="mb-3" nuxt :href="'https://www.intercontinental-academia.org/blog/' + podcast">
          <v-icon left>mdi-play-circle</v-icon>
          Podcast
        </v-btn>
      </template>
  ...
  async fetch() {
    if (this.item.podcast) {
      const rst = await this.$content('Blog')
        .where({
          path: this.item.podcast.substring(0, this.item.podcast.length - 3).substring(7),
        })
        .only('slug')
        .fetch()
