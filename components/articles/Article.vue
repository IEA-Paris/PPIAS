<template>
  <v-row class="article-container">
    <Toc v-if="item.toc.length" :toc="item.toc" :active-toc="currentlyActiveToc" />
    <v-card :id="(item.anchor && item.anchor.toLowerCase()) || item.post_title" class="pa-6 my-3" v-bind="$attrs">
      ...
      <v-card-title>
        <span class="text-h4" v-html="highlight(item.post_title, $route.query.search)"></span>
      </v-card-title>
