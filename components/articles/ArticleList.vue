...
export default {
  ...
  async fetch() {
    const rst = await getContent('articles', this.$content, this.$route.query, this.searchString || null, true)
    if (rst) {
      console.log('rst: ', rst)
  ...
  watch: {
    ...
    async searchString(searchString) {
      const rst = await getContent('articles', this.$content, this.$route.query, searchString || null, true)
      if (rst) {
        this.count = rst.count
        this.pages = rst.pages
        this.pinnedPost = rst.pinnedPost
