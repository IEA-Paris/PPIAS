...
export default async () => {
  /* 
  // start by fetching all the authors document
  let authorsDocs = await $content('authors').fetch()
  ...
  let articleAuthors = articles.map((article) =>
  ...
  articleAuthors = articleAuthors
    ...
    .filter((author) => {
      ...
      // if so we merge them and remove the related articleAuthor
      authorsDocs[referencedDocIndex] = { ...authorsDocs[referencedDocIndex], ...author }
      if (referencedDocIndex) return true
      return false
    })
  // if an author from article isn't present in the authors documents, create it and, backlink his/her article
 
  authorsDocs = [...authorsDocs, ...articleAuthors]
  console.log('READY HOOK')
  console.log('FILES', files)

  const author = files[0].authors[0]
  ...
  const extraFilePath = path.join('content/articles', author.lastname + '.md')
...
article_title: Test generated content
picture: ''
picture_copyright: ''
yt: ''
published: true
